...
*/
// STUDENT: H should std::hash<K> by default.
...
public:
    ...
    */
    std::pair<iterator, bool> insert(const value_type& val) {
        auto [prev, curr] = find_node(val.first);
        if (curr != nullptr) {  // key already exists
            return {make_iterator(curr), false};
        }
        size_t bucket_index = _hash_function(val.first) % bucket_count();
        Node* new_node = new Node(val);
        new_node->next = _buckets_array[bucket_index];
        _buckets_array[bucket_index] = new_node;
        _size++;
    ...
    */
    bool erase(const K& key) {
        ...
        if (prev == nullptr) {  // key is in the first node
            size_t bucket_index = _hash_function(key) % bucket_count();
            _buckets_array[bucket_index] = curr->next;
        }
        else {
            prev->next = curr->next;
        }
    ...
    */
    iterator erase(const_iterator pos) {
        auto [prev, curr] = find_node(pos->first);
        if (prev == nullptr) {  // key is in the first node
            size_t bucket_index = _hash_function(pos->first) % bucket_count();
            _buckets_array[bucket_index] = curr->next;
        }
        else {
            prev->next = curr->next;
    ...
    */
    iterator begin() {
        auto first_index = first_not_empty_bucket();
        if (first_index >= bucket_count()) {
            return end();
        }
        return iterator(&_buckets_array, _buckets_array[first_index], first_index);
    }
    ...
    */
    iterator end() {
        // point to an nonexist bucket
        return iterator(&_buckets_array, nullptr, bucket_count());
    }
    ...
    */
    iterator find(const K& key) {
        auto curr = find_node(key).second;
        if (curr == nullptr) {
            return end();
        }
...
private:
    ...
    */
    struct Node
    {
        ...
        */
        ...
        Node(value_type _value) : value(_value), next(nullptr) {}
    };
    ...
    */
    ...
    size_t first_not_empty_bucket() const {
        return std::find_if(_buckets_array.begin(), _buckets_array.end(), [](Node* n) { return n != nullptr; }) - _buckets_array.begin();
    }
    ...
    */
    iterator make_iterator(Node* curr) {
        return iterator(&_buckets_array, curr, _hash_function(curr->value.first) % bucket_count());
    }
