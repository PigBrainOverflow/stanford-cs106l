...
public:
    ...
    */
    HashMapIterator<Map, IsConst>& operator++() {   // prefix
        _node = _node->next;
        if (_node == nullptr) {
            ++_bucket_idx;
            while (_bucket_idx < _buckets_array->size() && (*_buckets_array)[_bucket_idx] == nullptr) {
                ++_bucket_idx;
            }
            if (_bucket_idx < _buckets_array->size()) {
                _node = (*_buckets_array)[_bucket_idx];
            }
        }
        return *this;
    }
    HashMapIterator<Map, IsConst> operator++(int) { // postfix
        HashMapIterator<Map, IsConst> copy(*this);
        ++(*this);
        return copy;
    }
